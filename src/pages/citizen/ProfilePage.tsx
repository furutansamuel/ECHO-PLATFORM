...
const ProfileHeader: React.FC<ProfileHeaderProps> = ({
  ...
  return (
    <div className="relative rounded-3xl overflow-hidden border border-white/20 dark:border-white/10 shadow-2xl bg-gradient-to-b from-emerald-950/20 via-background to-background">
      
      {/* Dynamic Cover Banner */}
      <div className="h-48 sm:h-56 md:h-64 lg:h-72 w-full relative overflow-hidden bg-emerald-950">
        <img
          src={activeBanner}
        ...
        <div className="absolute top-4 right-4 sm:top-6 sm:right-6 flex items-center gap-2">
          ...
          <Badge className="bg-emerald-500/20 text-emerald-300 border-emerald-400/30 backdrop-blur-md px-3 py-1 text-[11px] font-black uppercase tracking-wider rounded-full shadow-lg flex items-center gap-1.5">
      ...
      <div className="relative px-4 sm:px-6 md:px-8 pb-8 -mt-20 sm:-mt-24">
        <div className="flex flex-col items-center text-center">
          ...
          <div className="relative mb-4 group">
            <div className="p-1.5 rounded-3xl bg-white/20 dark:bg-black/30 backdrop-blur-xl border border-white/40 dark:border-white/15 shadow-2xl ring-1 ring-emerald-500/30">
              <Avatar className="h-28 w-28 sm:h-32 sm:w-32 md:h-36 md:w-36 rounded-2xl object-cover shadow-inner">
                ...
                <AvatarFallback className="bg-gradient-to-br from-emerald-600 to-teal-800 text-white text-3xl sm:text-4xl font-black rounded-2xl">
                  {userInitials}
                </AvatarFallback>
              </Avatar>
            </div>
            <div className="absolute bottom-2 right-2 h-5 w-5 rounded-full bg-emerald-500 border-2 border-background shadow-md" title="Active ECHO Sentinel" />
          </div>
          ...
          <div className="flex items-center justify-center gap-2 max-w-full px-2">
            ...
            <div className="p-1 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-600 dark:text-emerald-400" title="Verified ECHO Identity">
              <CheckCircle2 className="h-5 w-5 sm:h-6 sm:w-6 fill-emerald-500/20" />
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center justify-center gap-2 sm:gap-3 mt-4">
            <span className="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-full text-xs font-bold bg-emerald-500/10 border border-emerald-500/20 text-emerald-700 dark:text-emerald-300 backdrop-blur-md shadow-sm">
              <Leaf className="h-3.5 w-3.5 text-emerald-500" />
              {currentLevelName}
            </span>

            <span className="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-full text-xs font-bold bg-blue-500/10 border border-blue-500/20 text-blue-700 dark:text-blue-300 backdrop-blur-md shadow-sm">
              <ShieldCheck className="h-3.5 w-3.5 text-blue-500" />
              {verificationRate}% Trust Rate
            </span>

            {profile?.region && (
              <span className="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-full text-xs font-bold bg-amber-500/10 border border-amber-500/20 text-amber-700 dark:text-amber-300 backdrop-blur-md shadow-sm">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 w-full mt-8">
            <div className="p-4 rounded-2xl bg-white/40 dark:bg-white/5 border border-white/40 dark:border-white/10 backdrop-blur-md text-center shadow-sm hover:border-emerald-500/30 transition-all">
            ...
            <div className="p-4 rounded-2xl bg-white/40 dark:bg-white/5 border border-white/40 dark:border-white/10 backdrop-blur-md text-center shadow-sm hover:border-emerald-500/30 transition-all">
              <FileText className="h-5 w-5 mx-auto mb-1 text-teal-500" />
      ...
      <div className="absolute bottom-4 right-4 sm:bottom-6 sm:right-6 z-20">
        <div className="relative">
          <Button
            size="icon"
            className="h-12 w-12 rounded-full bg-emerald-600 hover:bg-emerald-500 text-white shadow-xl ring-4 ring-background transition-transform active:scale-95"
          ...
          {showQuickMenu && (
            <div className="absolute bottom-14 right-0 w-52 py-2 rounded-2xl bg-background/90 backdrop-blur-xl border border-white/20 dark:border-white/10 shadow-2xl space-y-1 animate-in fade-in slide-in-from-bottom-2">
              <button
                className="w-full px-4 py-2 text-left text-xs font-bold text-foreground hover:bg-emerald-500/10 flex items-center gap-2 transition-colors"
                onClick={() => {
...
const ReputationScoreCard: React.FC<ReputationScoreCardProps> = ({
  ...
  return (
    <Card className="border-emerald-500/20 bg-gradient-to-br from-emerald-950/20 via-card to-card backdrop-blur-xl shadow-xl rounded-3xl overflow-hidden">
      <CardContent className="p-6">
        <div className="flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="flex items-center gap-5 w-full md:w-auto">
            <div className="relative h-24 w-24 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 flex flex-col items-center justify-center shrink-0 shadow-inner">
            ...
            <div>
              ...
              <div className="flex items-center gap-1 mt-1 text-amber-500">
                {Array.from({ length: 5 }).map((_, i) => (
                  <Star
                    key={i}
                    className={`h-4 w-4 ${i < starsCount ? "fill-amber-500" : "text-muted/30"}`}
                  />
                ))}
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 gap-4 w-full md:w-auto border-t md:border-t-0 md:border-l border-border pt-4 md:pt-0 md:pl-6">
            <div>
              ...
              <p className="text-sm font-black text-emerald-500 flex items-center gap-1 mt-1">
...
const StreaksAndGoals: React.FC<StreaksAndGoalsProps> = ({
  ...
  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
    <Card className="border-amber-500/20 bg-amber-500/5 backdrop-blur-md rounded-3xl md:col-span-1 flex flex-col justify-between">
...
const AiCitizenInsights: React.FC<AiInsightsProps> = ({ insights }) => (
  <Card className="border-emerald-500/20 bg-gradient-to-br from-emerald-950/10 via-card to-card backdrop-blur-md rounded-3xl">
    <CardHeader className="pb-2">
      <div className="flex items-center gap-2 text-emerald-500">
...
const JourneyTimeline: React.FC<JourneyTimelineProps> = ({ events }) => (
  <Card className="border-white/10 bg-card shadow-lg rounded-3xl">
    ...
    <CardContent className="pl-6">
      <div className="relative border-l-2 border-emerald-500/30 space-y-6 ml-3">
        {events.map((event) => (
          <div key={event.id} className="relative pl-6">
            <div className="absolute -left-[17px] top-0 p-1.5 rounded-full bg-background border-2 border-emerald-500 text-emerald-500">
              {event.icon}
            </div>
            <div>
      ...
      <Tabs defaultValue="reports" className="w-full">
        ...
        <TabsContent value="certificates" className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {certificates.length > 0 ? (
            certificates.map((cert) => (
              <div key={cert.id} className="p-4 rounded-2xl border border-emerald-500/20 bg-emerald-500/5 space-y-2">
        ...
        <TabsContent value="map">
          <div className="h-48 w-full rounded-2xl bg-muted/40 border border-border flex flex-col items-center justify-center gap-2">
            <MapIcon className="h-8 w-8 text-emerald-500/50" />
...
const ShareCardModal: React.FC<ShareCardModalProps> = ({
  ...
  <Dialog open={open} onOpenChange={onOpenChange}>
    <DialogContent className="sm:max-w-md bg-gradient-to-br from-emerald-950 via-background to-black border-emerald-500/30 text-foreground rounded-3xl p-6">
      <DialogHeader>
        <DialogTitle className="text-center text-xl font-black text-emerald-400">
      ...
      <div className="p-6 rounded-2xl bg-white/10 backdrop-blur-xl border border-white/20 shadow-2xl text-center space-y-4">
        <Avatar className="h-20 w-20 mx-auto ring-2 ring-emerald-500">
          <AvatarImage src={profile?.avatar_url} />
          <AvatarFallback className="bg-emerald-700 text-white font-black">EC</AvatarFallback>
        </Avatar>

        <div>
          <h3 className="text-xl font-black text-white">{profile?.full_name || "ECHO Citizen"}</h3>
          <p className="text-xs font-bold text-emerald-300">{levelName}</p>
        </div>
        ...
        <div className="grid grid-cols-3 gap-2 border-t border-b border-white/10 py-3 text-center">
          <div>
            ...
            <p className="text-sm font-black text-emerald-400">{reputationScore}/100</p>
          </div>
          <div>
...
const ProfilePage: React.FC = () => {
  ...
  }, [profile?.full_name, impactStats.reportsSubmitted]);
