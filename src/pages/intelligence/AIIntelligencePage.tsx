...
function forecastBadge(value: number | undefined, invert = false) {
  ...
  if (bad) return { label: invert ? 'Low ↓' : 'High ↑', className: 'bg-red-100 text-red-700' };
  if (mid) return { label: 'Stable →', className: 'bg-yellow-100 text-yellow-700' };
  return { label: invert ? 'Good ↑' : 'Low ↓', className: 'bg-green-100 text-green-700' };
}
