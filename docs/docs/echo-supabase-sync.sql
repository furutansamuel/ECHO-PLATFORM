...
-- 13d. global_search
-- Signature and return shape match the frontend contract exactly:
-- GlobalSearchPage.tsx calls supabase.rpc('global_search', { p_query, p_content_type, p_limit })
-- and reads results.total_results, results.query, results.reports[].severity/reference_number/created_at.
-- The previous version used a different signature (search_term text) and a different
-- return shape (no total_results/query, missing report fields), so every call from
-- GlobalSearchPage failed as a param-mismatch RPC error.
