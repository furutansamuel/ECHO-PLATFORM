...
const statusConfig: Record<ReportStatus, { icon: React.ElementType; token: string; description: string }> = {
  ...
  'Under Review': { icon: Eye, token: 'info', description: 'Environmental officers are reviewing the report' },
  'Pending Verification': { icon: UserPlus, token: 'info', description: 'Awaiting field verification by authorized personnel' },
  ...
  'Closed': { icon: CheckCircle, token: 'status-safe', description: 'The case is officially closed' },
...
const tokenClasses: Record<string, { solid: string; soft: string }> = {
  'status-warning': { solid: 'bg-status-warning border-status-warning text-primary-foreground', soft: 'bg-status-warning/10 text-status-warning' },
  'status-safe': { solid: 'bg-status-safe border-status-safe text-primary-foreground', soft: 'bg-status-safe/10 text-status-safe' },
  'info': { solid: 'bg-info border-info text-primary-foreground', soft: 'bg-info/10 text-info' },
  'primary': { solid: 'bg-primary border-primary text-primary-foreground', soft: 'bg-primary/10 text-primary' },
  'destructive': { solid: 'bg-destructive border-destructive text-primary-foreground', soft: 'bg-destructive/10 text-destructive' },
  'muted-foreground': { solid: 'bg-muted-foreground border-muted-foreground text-primary-foreground', soft: 'bg-muted text-muted-foreground' },
};
  ...
  if (currentStatus === 'Rejected') {
    ...
    return (
      <div className="flex items-start gap-4 p-4 rounded-lg bg-destructive/5 border border-destructive/20">
        <div className="flex items-center justify-center w-8 h-8 rounded-full bg-destructive text-primary-foreground shrink-0">
