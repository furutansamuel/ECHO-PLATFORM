            ...
            <CardContent className="space-y-3">
              {isDemo && recentVolunteering.length > 0 ? (
                recentVolunteering.map((v) => (
                  <div key={v.id} className="flex items-center gap-3 p-2 rounded-lg bg-success-subtle">
                    ...
                    <Badge className="bg-success-subtle text-success border-none text-[9px]">Registered</Badge>
                  </div>
                ))
              ) : (
